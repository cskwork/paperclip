...
const externalStatusTranslationKeys: Record<string, string> = {
  failed: "tasks.filters.externalFailed",
  waiting: "tasks.filters.externalWaiting",
  running: "tasks.filters.externalRunning",
  auth_required: "tasks.filters.externalAuthRequired",
  unreachable: "tasks.filters.externalUnreachable",
  stale: "tasks.filters.externalStale",
  none: "tasks.filters.externalNone",
};
    ...
    <Popover>
      <PopoverTrigger asChild>
        <Button
          ...
          title={iconOnly ? t(activeFilterCount > 0 ? "tasks.filters.buttonActive" : "tasks.filters.button", { count: activeFilterCount }) : undefined}
          aria-label={iconOnly ? t(activeFilterCount > 0 ? "tasks.filters.buttonActive" : "tasks.filters.button", { count: activeFilterCount }) : undefined}
        >
          <Filter className={iconOnly ? "h-3.5 w-3.5" : "h-3.5 w-3.5 sm:h-3 sm:w-3 sm:mr-1"} />
          {!iconOnly && <span className="hidden sm:inline">{t(activeFilterCount > 0 ? "tasks.filters.buttonActive" : "tasks.filters.button", { count: activeFilterCount })}</span>}
      ...
      >
        <div className="space-y-3 p-3">
          ...
          <div className="space-y-1.5">
            <span className="text-xs text-muted-foreground">{t("tasks.filters.quick")}</span>
            <div className="flex flex-wrap gap-1.5">
              {issueQuickFilterPresets.map((preset) => {
                ...
                return (
                  ...
                  >
                    {t(preset.label === "All" ? "tasks.filters.all" : preset.label === "Active" ? "tasks.filters.active" : preset.label === "Backlog" ? "tasks.backlog" : "tasks.done")}
                  </button>
                );
              })}
            </div>
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
            <div className="min-w-0 space-y-3">
              <div className="space-y-1">
                <span className="text-xs text-muted-foreground">{t("tasks.filters.status")}</span>
                <div className="space-y-0.5">
                  {issueStatusOrder.map((status) => (
                    <label key={status} className="flex cursor-pointer items-center gap-2 rounded-sm px-2 py-1 hover:bg-accent/50">
                      ...
                      <span className="text-sm">{t(statusTranslationKeys[status] ?? "tasks.filters.status")}</span>
                    </label>
                  ))}
                </div>
              </div>
              ...
              <div className="space-y-1">
                <span className="text-xs text-muted-foreground">{t("tasks.filters.priority")}</span>
                <div className="space-y-0.5">
                  {issuePriorityOrder.map((priority) => (
                    <label key={priority} className="flex cursor-pointer items-center gap-2 rounded-sm px-2 py-1 hover:bg-accent/50">
                      ...
                      <span className="text-sm">{t(priorityTranslationKeys[priority] ?? "tasks.medium")}</span>
                    </label>
                  ))}
                </div>
              </div>
            ...
            <div className="min-w-0 space-y-3">
              <div className="space-y-1">
                <span className="text-xs text-muted-foreground">{t("tasks.filters.responsible")}</span>
                <div className="max-h-32 space-y-0.5 overflow-y-auto">
                  <label className="flex cursor-pointer items-center gap-2 rounded-sm px-2 py-1 hover:bg-accent/50">
                    ...
                    />
                    <span className="text-sm">{t("tasks.filters.unassigned")}</span>
                  </label>
                  {currentUserId ? (
                    <label className="flex cursor-pointer items-center gap-2 rounded-sm px-2 py-1 hover:bg-accent/50">
                      <Checkbox
                      ...
                      <span className="text-sm">{t("tasks.filters.me")}</span>
                    </label>
                  ) : null}
                  {(agents ?? []).map((agent) => (
              ...
              {creatorOptions.length > 0 ? (
                <div className="space-y-1">
                  <span className="text-xs text-muted-foreground">{t("tasks.filters.creator")}</span>
                      ...
                      {selectedCreatorOptions.map((creator) => (
                        <Badge key={creator.id} variant="secondary" className="gap-1 pr-1">
                          ...
                          <button
                            ...
                            aria-label={t("tasks.filters.removeCreator", { name: creator.label })}
                          >
                            <X className="h-3 w-3" />
                          </button>
                        </Badge>
                  ...
                  <div className="relative">
                    ...
                    <Input
                      ...
                      placeholder={t("tasks.filters.searchCreators")}
                  ...
                  <div className="max-h-32 space-y-0.5 overflow-y-auto">
                    {visibleCreatorOptions.length > 0 ? visibleCreatorOptions.map((creator) => {
                      ...
                      <div className="px-2 py-1 text-xs text-muted-foreground">{t("tasks.filters.noCreatorMatches")}</div>
                    )}
                  </div>
                </div>
              ) : null}
              ...
              {projects && projects.length > 0 ? (
                <div className="space-y-1">
                  <span className="text-xs text-muted-foreground">{t("tasks.filters.project")}</span>
                  <div className="max-h-32 space-y-0.5 overflow-y-auto">
            ...
            <div className="min-w-0 space-y-3">
              {labels && labels.length > 0 ? (
                <div className="space-y-1">
                  <span className="text-xs text-muted-foreground">{t("tasks.filters.labels")}</span>
                  <div className="max-h-32 space-y-0.5 overflow-y-auto">
                    {labels.map((label) => (
              ...
              {workspaces && workspaces.length > 0 ? (
                <div className="space-y-1">
                  <span className="text-xs text-muted-foreground">{t("tasks.filters.workspace")}</span>
                  <div className="max-h-32 space-y-0.5 overflow-y-auto">
              ...
              {enableExternalObjectFilters ? (
                <div className="space-y-1">
                  <span className="text-xs text-muted-foreground">{t("tasks.filters.externalStatus")}</span>
                  <div className="space-y-0.5">
                    {externalObjectFilterOrder.map((value) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <span className="text-sm">{t(externalStatusTranslationKeys[value] ?? "tasks.filters.externalStatus")}</span>
                        </label>
                      );
                    })}
                  </div>
              ...
              <div className="space-y-1">
                <span className="text-xs text-muted-foreground">{t("tasks.filters.visibility")}</span>
                <label className="flex cursor-pointer items-center gap-2 rounded-sm px-2 py-1 hover:bg-accent/50">
                  <Checkbox
                    checked={state.liveOnly}
                    onCheckedChange={(checked) => onChange({ liveOnly: checked === true })}
                  />
                  <span className="text-sm">{t("tasks.filters.liveOnly")}</span>
                </label>
                {enableRoutineVisibilityFilter ? (
                  <label className="flex cursor-pointer items-center gap-2 rounded-sm px-2 py-1 hover:bg-accent/50">
                    ...
                    />
                    <span className="text-sm">{t("tasks.filters.hideRoutines")}</span>
                  </label>
                ) : null}
              </div>
            </div>
